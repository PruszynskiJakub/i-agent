...
# Initialize agent service with all dependencies
agent_service = AgentService(state, services['openai'], services['db'], services['langfuse'])
...
def restore_conversation(conversation_uuid: str) -> list:
    ...
    """
    stored_messages = services['db'].get_messages(conversation_uuid)
...
async def main_loop(conversation_uuid: str, conversation_history: list, exit_keyword: str = 'exit') -> None:
    ...
    while True:
        ...
        trace = services['langfuse'].create_trace({
        ...
        try:
            # Store user message
            services['db'].store_message(conversation_uuid, "user", user_input)
            ...
            services['langfuse'].finalize_trace(
                trace,
